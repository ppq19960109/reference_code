...
typedef struct fdr_bmp_header{
    uint16_t bfType;
    uint32_t bfSize;
    uint16_t bfReserved1;
    uint16_t bfReserved2;
    uint32_t bfOffBits;
}__attribute__((packed))fdr_bmp_header_t;

typedef struct fdr_bmp_info{
    uint32_t  biSize;
    uint32_t  biWidth;
    uint32_t  biHeight;
    uint16_t  biPlanes;
...
}__attribute__((packed))fdr_bmp_info_t;

int fdr_yuv_save_to_bmp(const char  *filepath, uint8_t *rgb, int width, int heigh, int chs){
    fdr_bmp_header_t header = {};
    fdr_bmp_info_t   info = {};
    FILE *fj;
    size_t wlen;

    header.bfType = ('M' << 8) | 'B';
    header.bfSize = width * heigh * chs + sizeof(fdr_bmp_header_t) + sizeof(fdr_bmp_info_t);
    header.bfOffBits = sizeof(fdr_bmp_header_t) + sizeof(fdr_bmp_info_t);

    info.biSize = sizeof(fdr_bmp_info_t);
    info.biWidth = width;
    info.biHeight = -heigh;
    info.biPlanes = 1;
    ...
    fj = fopen(filepath, "w+");
    if(fj == NULL){
        log_warn("save_bmp_file => open file  %s fail\n", filepath);
        return -1;
    }

    wlen = fwrite(&header, 1, sizeof(header), fj);
    if(wlen  != sizeof(header)){
        log_warn("save_bmp_file => write header fail:%zu\n", wlen);
        fclose(fj);
        return -1;
    }
    ...
    if(wlen  != sizeof(info)){
        log_warn("save_bmp_file => write info fail:%zu\n", wlen);
        fclose(fj);
        return -1;
    }
    ...
    if(wlen  != width*heigh*chs){
        log_warn("save_bmp_file => write rgb fail:%zu\n", wlen);
        fclose(fj);
        return -1;
    }
