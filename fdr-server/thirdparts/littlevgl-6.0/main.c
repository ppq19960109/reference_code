...
#define CANVAS_HEIGHT 800
    ...
    lv_font_ft_set(24, 4);
    style.text.font  = &lv_font_freestyle;
    lv_canvas_draw_text(canvas, 64, 480, 400, &style, "abg中国", LV_LABEL_ALIGN_LEFT);

#else
    lv_obj_t *label_main = lv_label_create(lv_scr_act(), NULL);
    lv_label_set_long_mode(label_main, LV_LABEL_LONG_DOT);     /*Break the long lines*/
    lv_label_set_align(label_main, LV_LABEL_ALIGN_CENTER);       /*Center aligned lines*/
    lv_obj_set_pos(label_main, 10, 100);
    lv_obj_set_size(label_main, 200, 200);
    
    lv_label_set_body_draw(label_main, true);

    //lv_font_ft_set(18, 4);
    //style.text.font  = &lv_font_freestyle;
    lv_label_set_style(label_main, LV_LABEL_STYLE_MAIN, &lv_style_plain);
    lv_label_set_text(label_main, "abg中国");
#endif
