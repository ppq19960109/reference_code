...
int32_t uart_read(int32_t fd, uint8_t *buf, uint32_t len)
{
    ...
    if(retval < 0) {
        hal_error("Error: select fd read"
			"%d: %s\r\n", fd, strerror(errno));
        return -1;
    } else if(0 == retval) {
        return 0;
...
int32_t uart_write(int32_t fd, uint8_t *buf, uint32_t len)
{
    ...
    if(retval < 0) {
        hal_error("Error: select fd write"
			"%d: %s\r\n", fd, strerror(errno));
        return -1;
    } else if(0 == retval) {
        hal_error("select fd write not ready, fd = %d\r\n", fd);
        return 0;
    }
    
    wlen = write(fd, buf, len);
    if(wlen < 0) {
        hal_error("write err: %d\r\n", wlen);
    }
    return wlen;
}
...
static int32_t uart_set_opt(int32_t fd, uint32_t baudrate, uint8_t nbits, uint8_t parity, uint8_t stop)
{
    ...
    if(tcgetattr(fd, &attr) < 0) {
        hal_error("failed to get uart attibute\r\n");
        return -1;
    }
    ...
    if(tcsetattr(fd, TCSANOW, &attr) < 0) {
        hal_error("Error: failed to set uart attibute.\r\n");
        return -1;
    }
...
int32_t uart_open(uint8_t uartx, uint32_t baudrate, uint8_t nbits, uint8_t parity, uint8_t stop)
{
    ...
    if(HAL_UART_MAX <= uartx) {
        hal_error("unknow uart %d\r\n", uartx);
        return -1;
    }
    ...
    if (fd < 0) {
		hal_error("Error: Could not open uart "
			"'%s': %s\r\n", path, strerror(errno));
		if (errno == EACCES) {
			hal_error("Run as root?\r\n");
		}
        goto end;
	}
    ...
    if(uart_set_opt(fd, baudrate, nbits, parity, stop) < 0) {
        hal_error("Error: set opt failed, "
			"`%s': %s\r\n", path, strerror(errno));
    }

    hal_info("open %s, fd = %d\r\n", path, fd);
    return fd;
...
{
    if(close(fd) < 0) {
        hal_error("close uart dev failed, fd = %d, %s\r\n", fd, strerror(errno));
        return -1;
    }
