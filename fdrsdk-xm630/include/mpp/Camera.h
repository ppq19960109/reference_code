...
/*****************************************
u8LedMode: (LED类型用于接口camera_get_ledMode的返回)
...
/*************************************************************************
函数功能:		配置AE属性
输入参数:		u8Mode: 模式  (0:Auto  1:Manual)
			u32SnsShut: sensor曝光行数(针对Manual模式)
			u32SnsAGain: sensor模拟增益(针对Manual模式)
			u32SnsDGain: sensor数字增益(针对Manual模式)
			u32IspDGain: isp数字增益(针对Manual模式)
输出参数:	无
返回参数:	0:	成功
				-1: 出错
注:
	增益精度为1024
*************************************************************************/
int camera_set_ae_attr(XM_U8 u8Mode, 
								XM_U32 u32SnsShut, 
								XM_U32 u32SnsAGain, 
								XM_S32 u32SnsDGain, 
								XM_U32 u32IspDGain);
...
注:
	增益精度为1024
*************************************************************************/
int camera_get_ae_attr(XM_U8 *pu8Mode, 
								XM_U32 *pu32SnsShut, 
								XM_U32 *pu32SnsAGain, 
								XM_U32 *pu32SnsDGain, 
								XM_U32 *pu32IspDGain);
